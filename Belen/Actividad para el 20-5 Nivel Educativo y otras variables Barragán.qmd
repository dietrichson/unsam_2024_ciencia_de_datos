...
En la siguiente tabla, se puede observar en base a la media (promedio de ingresos por región), que los ingresos más altos pertenecen a la region de la Patagonia (con un ingreso promedio de \$563456), dejando al Noreste como la región con menor ingreso promedio (con \$373542).
...
```{r}
##10 hacer subdivision (amba/interior)
temp_df |> 
      group_by(NIVEL_ED, REGION2) |>
      summarize(
        Mediana = median(ITF)
      ) |> 
   ggplot(aes(NIVEL_ED,Mediana, color = REGION2))+
  geom_line()
```
